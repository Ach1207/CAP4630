...
class Checker(TwoPlayerGame):
    ...
    def __init__(self, players):
        ...
        # self.board = np.arrange(8*8).reshape(8,8)
        self.blank_board = np.zeros((8,8), dtype= object)

        # initialize blank board
        self.board = self.blank_board.copy()

        # initialization of start of black pieces
        self.black_pieces = [           
            (0,1), (0,3), (0,5), (0,7),
            (1,0), (1,2), (1,4), (1,6)
        ]
        # initialization of start of white pieces
        self.white_pieces = [
            (6,1), (6,3), (6,5), (6,7),
            (7,0), (7,2), (7,4), (7,6)
        ]
        
        #initialize B to stand for current move of player using black pieces
        for i,j in self.black_pieces:
            self.board[i,j] = "B"
        #initialize W to stand for current move of player using white pieces
    ...
    def lose(self):
        ...
        if self.current_player-1 == 0:
            if "W" in [(0,1), (0,3), (0,5), (0,7)]:
                return True
        elif self.current_player-1 == 1:
            if "B" in [(7,0), (7,2), (7,4), (7,6)]:
                return True
        else:
            return False
